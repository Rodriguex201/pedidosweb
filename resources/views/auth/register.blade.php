    ...
    <main class="flex min-h-screen items-center justify-center px-4 py-8 sm:px-6 lg:px-8">
        <section class="w-full max-w-lg rounded-2xl border border-slate-200/80 bg-white/90 p-6 shadow-xl shadow-slate-200/60 backdrop-blur sm:p-8">
            ...
            <form method="POST" action="{{ route('register.submit') }}" class="space-y-5" novalidate>
                ...
                <div>
                    ...
                    <input id="name" name="name" type="text" autocomplete="name" value="{{ old('name') }}" required
                        class="block w-full rounded-xl border px-4 py-2.5 text-sm shadow-sm outline-none transition placeholder:text-slate-400 focus:ring-4 {{ $errors->has('name') ? 'border-rose-400 bg-rose-50/50 focus:border-rose-500 focus:ring-rose-100' : 'border-slate-300 bg-white focus:border-emerald-500 focus:ring-emerald-100' }}"
                ...
                <div>
                    ...
                    <input id="password_confirmation" name="password_confirmation" type="password" autocomplete="new-password" required
                        class="block w-full rounded-xl border px-4 py-2.5 text-sm shadow-sm outline-none transition placeholder:text-slate-400 focus:ring-4 {{ $errors->has('password_confirmation') ? 'border-rose-400 bg-rose-50/50 focus:border-rose-500 focus:ring-rose-100' : 'border-slate-300 bg-white focus:border-emerald-500 focus:ring-emerald-100' }}"
                ...
                <div>
                    ...
                    <input id="company_code" name="company_code" type="text" autocomplete="organization" value="{{ old('company_code') }}" required
                        class="block w-full rounded-xl border px-4 py-2.5 text-sm uppercase shadow-sm outline-none transition placeholder:text-slate-400 focus:ring-4 {{ $errors->has('company_code') ? 'border-rose-400 bg-rose-50/50 focus:border-rose-500 focus:ring-rose-100' : 'border-slate-300 bg-white focus:border-emerald-500 focus:ring-emerald-100' }}"
                        placeholder="A001 - B001">
                    @error('company_code')
                        <p class="mt-1.5 text-xs font-medium text-rose-600">{{ $message }}</p>
                    @enderror
                </div>
            ...
            <p class="mt-7 text-center text-sm text-slate-600">
                ¿Ya tienes cuenta?
                <a href="{{ route('login') }}" class="font-semibold text-emerald-600 hover:text-emerald-700 hover:underline">Iniciar sesión</a>
            </p>
        </section>
    </main>
</body>
